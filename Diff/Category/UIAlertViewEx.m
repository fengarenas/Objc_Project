...
@implementation UIAlertView(UIAlertViewEx)

static char UIAlertViewExDelegateKey;
...
- (void)showAlertWithCompleteBlock:(CompleteBlock)block
{
    if (block) {
         ...
         */
        objc_setAssociatedObject(self, &UIAlertViewExDelegateKey, block, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        ////设置delegate
        self.delegate = self;
    }
    ////show
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ///获取关联的对象，通过关键字。
    CompleteBlock block = objc_getAssociatedObject(self, &UIAlertViewExDelegateKey);
    if (block) {
        ///block传值
        block(buttonIndex);
    }
